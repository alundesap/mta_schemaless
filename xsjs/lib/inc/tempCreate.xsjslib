// $.import("mta_schemaless.js","crudCommon");
...
// 04 "created" LONGDATE CS_LONGDATE NOT NULL , 
    ...
    pStmt = param.connection.prepareStatement('select "mta_schemaless.db.data::tempId".NEXTVAL from dummy');
    ...
    for (var i=0; i<2; i++) {
        ...
        if (i<1) {
            pStmt = param.connection.prepareStatement('insert into "mta_schemaless.db.data::sensors.temp" values(?,?,TO_TIMESTAMP (?, \'YYYY-MM-DD HH24:MI:SS.FF3\'),CURRENT_UTCTIMESTAMP)');
        }
        else {
